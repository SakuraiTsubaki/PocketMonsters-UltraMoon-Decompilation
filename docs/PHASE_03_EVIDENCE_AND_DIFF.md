...
- Narrow Ultra Moon/Ultra Sun code differences without embedding executable bytes in reports.
...
- paired-version `.code` when performing Ultra Moon ↔ Ultra Sun comparison
...
5. Compare Ultra Moon ↔ Ultra Sun only when both exact target revisions are known.
