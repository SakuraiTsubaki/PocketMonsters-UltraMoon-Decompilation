...
- [ ] Inventory the first verified Ultra Moon target
...
Run `tools/inventory_target.py` on the first local Ultra Moon target or extracted tree, register exact identity, then begin NCCH/ExeFS/RomFS structural mapping. Retail bytes and keys remain local.
